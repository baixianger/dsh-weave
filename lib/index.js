...
export class DshWeaveTransport {
  ...
  async #accept(incoming) {
    ...
    try {
      ...
      // Protocol consumers get first refusal. This lets a higher-level protocol
      // (such as dsh-chat room synchronization) consume its own frames without
      // injecting opaque transport JSON into an agent conversation.
      let claimed = false;
      for (const listener of this.listeners) if (await listener(received) === true) claimed = true;
