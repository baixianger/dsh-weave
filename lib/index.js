...
export const inject = ["connection", "tools", "sessions", "sessionTitle", "sessionPersistence", "workspaceRegistry"];
...
export class DshWeaveTransport {
  ...
  hosts() { return this.peers().map((hostId) => ({ hostId, addressKnown: this.peerTickets.has(hostId) })); }
  ...
  async sendTo({ hostId, from, to, text, id }) {
    ...
    return this.send({ ticket, from, to, text, id });
  }
  async sessionCatalog() {
    ...
    for (const header of headers ?? []) {
      ...
      items.push({ id, title: title || id, running: Boolean(active), updatedAt });
    }
  ...
  async remoteSessions(timeoutMs = 3_000) {
    const catalogs = await Promise.all(this.endpoints().map(async ({ peerId }) => {
      try {
        ...
        return { hostId: peerId, hostName: String(sent.result.hostName || peerId.slice(0, 8)), workspaces: sent.result.workspaces };
  ...
  async #accept(incoming) {
    ...
    try {
      ...
      if (!claimed && bridge) bridge.deliverExternal(`weave:${peerId}:${message.from}`, message.to, message.text, { id: message.id, transport: "weave" });
